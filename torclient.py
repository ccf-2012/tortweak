...
class TrDownloadClient(DownloadClientBase):
    ...
    def loadTorrents(self, seeding=True, paused=False):
    ...
    def setLocation(self, tor_hash, location):
        try:
            ...
            if trTor:
                trTor.locate_data(location)
                s = 'Torrent successfully re-located!'
                logger.info(s)
                if self.log:
                    self.log.message(s)
                st = self.mkSeedTor(trTor)
